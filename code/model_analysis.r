...
# plot inertia
create_inertia_plot <- function(model, elbow_point = NULL) {
  if (!dir.exists("./results")) {
    dir.create("./results")
  }
  plot_file <- sprintf("./results/Inertia-Plot_%s.png", model$name)
  png(file = plot_file, width = 9000, height = 6000, units = "px", res = 900)
  
  par(mar = c(9, 6, 4, 2), mgp = c(4, 1, 0))
  
  # Calculate inertia values
  inertia_values <- sort(model$clust$height, decreasing = TRUE)[1:20]
  num_clusters <- 1:length(inertia_values)
  
  # Apply loess smoothing to the inertia values
  loess_fit <- loess(inertia_values ~ num_clusters, span = 0.3)
  smooth_inertia <- predict(loess_fit, num_clusters)
  
  # Calculate the second derivative on the original inertia values to find the elbow point
  second_derivative <- diff(diff(inertia_values))
  if (is.null(elbow_point)) {
    elbow_point <- which.min(second_derivative) + 1
  }
  
  
  # Plot the original inertia values
  plot(num_clusters, inertia_values, type = "o", 
       col = "blue", lwd = 2, pch = 19, cex = 1.2,
       main = "Inertia Plot of Model by Number of Clusters",
       xlab = "Number of Clusters", ylab = "Inertia",
       xaxt = 'n', yaxt = 'n', bty = 'l')
  
  # Add smoothed inertia line
  lines(num_clusters, smooth_inertia, col = "darkgrey", lwd = 2, lty = 2)
  
  # Customize x and y axes
  axis(1, at = num_clusters, labels = num_clusters, las = 2, cex.axis = 0.8)
  axis(2, las = 1, cex.axis = 0.8)
  
  # Add grid lines
  grid(nx = NULL, ny = NULL, col = rgb(0.5, 0.5, 0.5, 0.5), lty = "dotted")
  
  # Mark the elbow point on the original inertia line
  if (!(is.na(elbow_point))) {
    points(elbow_point, inertia_values[elbow_point], col = "red", pch = 19, cex = 1.5)
    text(elbow_point, inertia_values[elbow_point], labels = paste("Elbow at", elbow_point, "clusters"), pos = 4, col = "red")
    # Add legend
    legend("topright", legend = c("Inertia", "Smoothed Inertia", "Elbow Point"), col = c("blue", "darkgrey", "red"), lwd = 2, lty = c(1, 2, 1), pch = 19, bty = "n")
  } else {
    # Add legend
    legend("topright", legend = c("Inertia", "Smoothed Inertia"), col = c("blue", "darkgrey"), lwd = 2, lty = c(1, 2), pch = 19, bty = "n")
  }
  print(sprintf("Inertia Plot saved here: %s", plot_file))
  #dev.off()
  }
